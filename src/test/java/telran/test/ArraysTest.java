...
@Test
    ...
    void testMatchesRules() {
        ...
        CharacterRule mustBeDigit = new CharacterRule(true, Character::isDigit, "Must contain at least one digit.");
        CharacterRule mustBeDot = new CharacterRule(true, c -> c == '.', "Must contain at least one dot (.)");
        CharacterRule mustNotBeSpace = new CharacterRule(false, Character::isWhitespace, "Space is disallowed.");
        CharacterRule[] mustBeRules = {mustBeUpperCase, mustBeLowerCase, mustBeDigit, mustBeDot};
        CharacterRule[] mustNotBeRules = {mustNotBeSpace};
        char[] test1 = {'a', 'n', '*', 'G', '.', '.', '1'};
        char[] test2 = {'a', 'n', '*', 'G', '.', '.', '1', ' '};
        char[] test3 = {'a', 'n', '*', '.', '.', '1'};
        char[] test4 = {'a', 'n', '*', 'G', '.', '.'};
        assertEquals("", matchesRules(test1, mustBeRules, mustNotBeRules));
        assertEquals("Space is disallowed.", matchesRules(test2, mustBeRules, mustNotBeRules));
        assertEquals("Must contain at least one uppercase letter.", matchesRules(test3, mustBeRules, mustNotBeRules));
        assertEquals("Must contain at least one digit.", matchesRules(test4, mustBeRules, mustNotBeRules));
    }
}
